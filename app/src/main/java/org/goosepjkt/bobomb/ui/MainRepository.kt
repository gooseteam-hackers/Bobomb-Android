...
import org.goosepjkt.bobomb.BuildVars.AttackSpeed
import org.goosepjkt.bobomb.services.core.Service
import org.goosepjkt.bobomb.services.repository.DslServicesRepository
import org.goosepjkt.bobomb.worker.AuthProxy
...
class MainRepository(private val context: Context?) {
    ...
    fun getAllRepositories(client: OkHttpClient): List<() -> List<Service>> = buildList {
        ...
        add {
            val services = DslServicesRepository.loadFromAssets(context!!, "services.dsl")
            android.util.Log.d("MainRepository", "Loaded ${services.size} services from DSL")
            services
        }
    }
    ...
    fun getTotalServicesCount(countryCode: String): Int {
        val allServices = DslServicesRepository.loadFromAssets(context!!, "services.dsl")
